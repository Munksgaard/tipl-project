    ...
    iter' (i-1) (set i new_r rs) cs ext adjs waas
      where
        ...
        rhs = if i < 2 then
                ...
                rhs'
\end{verbatim}
Apart from the difference in method, the semantics are slightly different, as
\verb+gauss+ only computes one iteration at a time. Furthermore, we don't apply
relaxation when using the Gauss-Seidel model, which might also explain why our
results never converge to a single value. Instead, after a number of
iterations, the Gauss-Seidel method seems to settle on a handful of different
configurations, which it then cycles between. This is to be expected, given the
nature of the Gauss-Seidel method. However, it is very possible that some form
of relaxation could help alleviate that problem.
...
After this preprocessing step of the elements we then do the remaining addition and summation
